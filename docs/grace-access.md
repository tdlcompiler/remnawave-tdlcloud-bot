...
GRACE_ACCESS_RESET_TRAFFIC_ON_START=false
...
Из-за этого панель и клиентское приложение показывают почти исчерпанный лимит
(«64.76 GiB из 65.76 GiB — 98%»), хотя доступна ровно квота. Если это мешает, включите
`GRACE_ACCESS_RESET_TRAFFIC_ON_START=true` (кабинет: «Grace-доступ → Что получает человек»):
при выдаче счётчик обнуляется, лимит становится равен квоте, и человек видит «0 из 1 ГБ».
Обнуление касается только истёкших подписок с безлимитным трафиком — там счётчик чисто
информационный. Расход до grace при этом теряется и не возвращается по окончании. Подписки,
упёршиеся в лимит трафика, не обнуляются никогда: это вернуло бы исчерпанную квоту тарифа.
...
6. Проверьте первого выданного клиента в Remnawave: статус временно `ACTIVE`, назначен только grace-сквад, внешний сквад снят, срок равен времени окончания grace, а лимит равен текущему расходу плюс `GRACE_ACCESS_TRAFFIC_GB`. Использованный трафик не должен уменьшиться. С `GRACE_ACCESS_RESET_TRAFFIC_ON_START=true` у истёкшей безлимитной подписки вместо этого расход обнулён, а лимит равен `GRACE_ACCESS_TRAFFIC_GB`.
...
Мониторинг перед гашением истёкшей подписки спрашивает панель «может, продлили в обход бота?».
Во время grace ACTIVE в панели — это оверлей, а не продление: признак `subscriptions.grace_session_open`
хранилище фиксирует **до** того, как оверлей уходит в панель, а мониторинг перечитывает подписку из
базы **после** снимка панели — любой оверлей, который он может увидеть, он видит вместе с признаком.
Гашение — один условный UPDATE (подписка всё ещё `active` с прошедшей датой), поэтому продление,
записанное в ту же секунду, не затирается. Сторож `tests/contracts/test_projection_reads_grace_marker_after_snapshot.py`
требует того же от любого переноса «панель → бот».

Снимок панели, снятый при открытой сессии, может обрабатываться уже после её закрытия (досрочный
откат, конфликт, снимок полного прохода синхронизации) — признак к этому моменту снят. Поэтому
хранилище в той же транзакции пишет и `subscriptions.grace_overlay_expire_at` — дату оверлея
(«конец grace» с точностью до миллисекунд), и при закрытии её не стирает: снимок с этой датой всегда
оверлей, а не продление, и ни проекция, ни мониторинг его в подписку не переносят. Миграция 0122
заполняет колонку по последней сессии каждой подписки.

v4.10–4.11 этого не делали: оверлей (сквад grace, лимит «расход + квота», дата конца grace) оседал
в подписке, а сессия закрывалась «человек продлил». Такие подписки лечит любое продление: перед
расчётом нового срока оно берёт из истории grace-сессий (`billing_before`) то, что было до grace, и
возвращает ровно те поля, где стоит значение оверлея; новый срок считается от настоящего окончания.
Если человек не продлил сразу и получил второй grace, источником берётся последний снимок, который
сам не оверлей.

Испорченную подписку узнают по одному из двух признаков, и каждый может дать только grace:
дата окончания равна дате оверлея (с точностью до миллисекунд), либо серверы подписки — ровно сквады
оверлея, ни один из них не продаётся (не доступен к покупке, не пробный, не входит ни в один тариф),
а grace хоть раз закрылся «оплатой». Второй признак — для тех, кто заплатил ещё на v4.10–4.11: дата
уже ушла от конца grace, а сквад остался. Их дату продление не трогает — эти дни куплены. Сквад
grace, который где-то продаётся, и сквад grace рядом с купленным сервером без совпадения даты —
не признак: на них можно быть честно.
...
приходит сообщение в бота (а пользователю без Telegram — письмо): подписка закончилась, но на N часов
оставлен доступ только к тому, что оператор назвал в `GRACE_ACCESS_ALLOWED_SERVICES` («Telegram»,
«Telegram и личный кабинет», «сайт проекта» — сквад grace пропускает то, что настроено на нодах, бот об
этом только сообщает), и M ГБ, чтобы продлить, — с кнопкой продления; и второе, когда доступ закрылся без
оплаты. Письма `grace_access_granted` / `grace_access_ended` редактируются в разделе «Email-шаблоны»
(переменные `{allowed}`, `{hours}`, `{traffic_gb}`, `{until}`, `{reason}`, `{tariff_name}`). Всё, что
подставляется в разметку Telegram и письма, экранируется. Выключатели `GRACE_ACCESS_NOTIFY_ADMINS` и
`GRACE_ACCESS_NOTIFY_USER` и фраза «что доступно» — в разделе «Grace-доступ» кабинета.
