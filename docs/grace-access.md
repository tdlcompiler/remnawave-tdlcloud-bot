...
В режиме `true` трафик должен быть не меньше 1 ГиБ. Для `limited` это дополнительный объём поверх уже использованного трафика, а не сброс счётчика.
...
6. Проверьте первого выданного клиента в Remnawave: статус временно `ACTIVE`, назначен только grace-сквад, внешний сквад снят, срок равен времени окончания grace. Использованный трафик не должен уменьшиться.
