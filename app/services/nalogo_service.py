        ...
        try:
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(receipt_url, proxy=proxy_url) as resp:
                    ...
                    # Быстрая проверка на слишком большой файл до скачивания
                    expected_size = int(resp.headers.get('Content-Length', 0))
                    if expected_size > _RECEIPT_MAX_BYTES:
                        logger.warning('Печатная форма чека NaloGO слишком велика (по заголовку)', content_length=expected_size)
                        return None

                    # Читаем данные потоком в цикле, чтобы забрать весь файл целиком (фикс апстрима)
                    chunks = []
                    total_downloaded = 0
                    ...
                    # Проверяем целостность скачанного файла по Content-Length
